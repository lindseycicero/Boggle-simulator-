...
boggle_t * bg_init(int seed, char * dict_fn){
  ...
  bg->dict = hm_init();
  FILE * dict = fopen(dict_fn,"r");
  char word[1024];
  while(fscanf(dict, "%1023s", word) != EOF){
...
void _bg_all_words(boggle_t *bg, hashmap_t * hm, int r, int c,
  ...
  if(r>=BOGGLE_DIMENSION||c>=BOGGLE_DIMENSION){
  return; 
  }
  if(r<0||c<0){
  return; 
  ...
  char current=bg->board[r][c];   
  if(current=='Q'){
    if(strlen(last_word)<=7){
    cur_word[strlen(cur_word)]=current;
    current='U';
    }else{
      return;
    }
  ...
  if(hm_check(bg->dict,cur_word)&&strlen(cur_word)>=3){
